    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // If we're using Qwen as primary, validate it has meaningful content
          if (primaryProvider === 'qwen') {
            // Qwen now returns sections-based structure like Gemini
            const sections = generatedContent?.sections || [];
            const hasTitle = generatedContent?.title && generatedContent.title.length > 5;
            const hasValidSections = sections.length >= 4; // warmup, reading, vocabulary, discussion minimum
            
            // Check for key sections
            const hasWarmup = sections.some((s: any) => s.type === 'warmup');
            const hasReading = sections.some((s: any) => s.type === 'reading' && s.paragraphs && Array.isArray(s.paragraphs) && s.paragraphs.length >= 2);
            const hasVocabulary = sections.some((s: any) => s.type === 'vocabulary' && s.words && Array.isArray(s.words) && s.words.length >= 3);
            const hasDiscussion = sections.some((s: any) => s.type === 'discussion' && s.questions && Array.isArray(s.questions) && s.questions.length >= 2);
            
            const hasMeaningfulContent = hasTitle && hasValidSections && hasWarmup && hasReading && hasVocabulary && hasDiscussion;
            
            if (!hasMeaningfulContent) {
              console.log("Qwen returned insufficient content, will try fallback");
              console.log("Validation results:", { hasTitle, hasValidSections, hasWarmup, hasReading, hasVocabulary, hasDiscussion });
              throw new Error("Insufficient content generated by Qwen");
            }
          }
        ...
        console.log(`Lesson generation completed in ${timeTaken.toFixed(1)} seconds${usedFallbackProvider ? ' (used fallback provider)' : ''}`);
        ...
        // If primary provider had an error, include it in the logs
        if (primaryProviderError) {
          console.error("Primary provider error was:", primaryProviderError);
        }
        
        // Refund the credit since both providers failed (skip for admin users)
        if (!user.isAdmin) {
          console.log(`Refunding credit to user ${user.id} due to generation failure`);
          // No actual deduction happened yet, so no need to refund
        }
        
        // Return error to client with status 503 (Service Unavailable)
        return res.status(503).json({ 
          message: "AI service unavailable - both primary and fallback providers failed", 
          error: aiError.message,
          primaryError: primaryProviderError ? primaryProviderError.message : null
        });
      }
    } catch (error: any) {
      if (error instanceof z.ZodError) {
