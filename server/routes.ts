...
export async function registerRoutes(app: Express): Promise<Server> {
  // Dynamic AI service loader - using Qwen via OpenRouter for lesson generation
  ...
  app.post("/api/lessons/generate", ensureAuthenticated, async (req, res) => {
    try {
      ...
      // 🚀 FULLY FREE LAUNCH (3 months): No credit requirements for any users
      // 📅 Future freemium transition (after 3 months):
      // - Free users: pay with credits OR implement daily limits
      // - Paid users: keep current credit system
      // - Images: become premium feature for free users
      console.log(`User ${user.id} generating lesson (free tier: ${user.subscriptionTier})`);
      ...
      try {
        ...
        const primaryProvider = validatedData.aiProvider || 'qwen'; // Default to Qwen

        console.log(`Using AI provider: ${primaryProvider}`);

        // FULLY FREE FOR NOW: Enable images for all users during initial launch
        // Future freemium: canGenerateImages = user.subscriptionTier === 'free' ? false : validatedData.generateImages

        // Generate lesson using AI (with images enabled for everyone)
        const aiService = await getAIService();
        generatedContent = await aiService.generateLesson(validatedData);
          
        // No fallback needed since we're only using one provider
        
        // Calculate time taken
        const endTime = Date.now();
        const timeTaken = (endTime - startTime) / 1000;
        console.log(`Lesson generation completed in ${timeTaken.toFixed(1)} seconds using AI`);
        ...
        // FULLY FREE FOR NOW: No credit deduction during initial launch
        // Future freemium: deduct credits for paid users only
        console.log(`Lesson generated successfully for user ${user.id} (no credits deducted)`);
        ...
        // FULLY FREE FOR NOW: No credits to refund during initial launch
        console.log(`Lesson generation failed for user ${user.id} (no credits to refund)`);
