      ...
      try {
        ...
        try {
          ...
          if (primaryProvider === 'qwen') {
            // Qwen returns flat structure with title, warmup, reading, vocabulary, etc.
            const hasTitle = generatedContent?.title && generatedContent.title.length > 5;
            const hasReading = generatedContent?.reading?.content && generatedContent.reading.content.length > 50;
            const hasVocabulary = generatedContent?.vocabulary?.words && Array.isArray(generatedContent.vocabulary.words) && generatedContent.vocabulary.words.length >= 3;
            const hasDiscussion = generatedContent?.discussion?.questions && Array.isArray(generatedContent.discussion.questions) && generatedContent.discussion.questions.length >= 2;
            
            const hasMeaningfulContent = hasTitle && hasReading && hasVocabulary && hasDiscussion;
            
            if (!hasMeaningfulContent) {
              console.log("Qwen returned insufficient content, will try fallback");
              console.log("Validation results:", { hasTitle, hasReading, hasVocabulary, hasDiscussion });
