...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/lessons/generate", ensureAuthenticated, async (req, res) => {
    try {
      ...
      if (!freeTrialActive && !user.isAdmin && user.credits < 1) {
      ...
      try {
        ...
        // Deduct credit (skip for admin users or during free trial)
        if (!freeTrialActive && !user.isAdmin) {
  ...
  app.post("/api/create-payment-intent", ensureAuthenticated, async (req, res) => {
    if (isFreeTrialActive()) {
      return res.status(403).json({ message: "Payments are disabled during the free trial period." });
    }
    try {
